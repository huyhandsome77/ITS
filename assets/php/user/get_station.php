...
$stmt = $conn->prepare("
    ...
    GROUP BY s.station_id, s.station_name, s.address, s.city, s.district, s.latitude, s.longitude
...
<div id="stationDataSource" class="hidden">
    ...
    <div class="station-card" data-id="<?= htmlspecialchars($s['station_id']) ?>"
        ...
        data-lat="<?= htmlspecialchars($s['latitude'] ?? '') ?>" data-lng="<?= htmlspecialchars($s['longitude'] ?? '') ?>"
