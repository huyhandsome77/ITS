    ...
    <script>
            ...
            fetch(`../../../php/admin/get_review_detail.php?review_id=${id}`)
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        const replyContent = review.reply ? 
                            `<div class="mt-4 bg-blue-50 p-4 rounded-lg border border-blue-100">
                                <p class="text-sm font-semibold text-blue-800 mb-1">💬 Admin trả lời:</p>
                                <p class="text-gray-700 text-sm whitespace-pre-wrap">${review.reply}</p>
                                <p class="text-xs text-blue-500 mt-2 text-right">Lúc: ${new Date(review.replied_at).toLocaleString('vi-VN')}</p>
                             </div>` : 
                            '<p class="text-sm text-gray-500 italic mt-4 text-center">Chưa có phản hồi từ Admin</p>';
                        
                        // Icon mapping based on type
                        let typeIcon = '🚗';
                        if(review.review_type === 'STATION') typeIcon = '🏢';
                        if(review.review_type === 'SERVICE') typeIcon = '🛠️';
                            ...
                            html: `
                                <div class="text-left font-[Inter]">
                                    ...
                                    <div class="flex items-center justify-between border-b pb-3 mb-4">
                                        <div>
                                            <h3 class="text-xl font-bold text-gray-800">Chi tiết đánh giá #${id}</h3>
                                            <p class="text-sm text-gray-500">${new Date(review.created_at).toLocaleString('vi-VN')}</p>
                                        </div>
                                        <span class="px-3 py-1 bg-gray-100 rounded-full text-xs font-semibold text-gray-600 uppercase tracking-wider">${review.status}</span>
                                    </div>
                                    ...
                                    <div class="flex items-center space-x-3 mb-4 p-3 bg-gray-50 rounded-lg">
                                        <div class="w-10 h-10 bg-indigo-100 text-indigo-600 rounded-full flex items-center justify-center font-bold">
                                            ${review.full_name ? review.full_name.charAt(0).toUpperCase() : '?'}
                                        </div>
                                        <div>
                                            <p class="font-semibold text-gray-800">${review.full_name}</p>
                                    ...
                                    <div class="mb-4">
                                        <div class="flex items-center mb-2">
                                            <span class="text-2xl mr-2">${'⭐'.repeat(review.rating)}</span>
                                            <span class="text-gray-400 text-sm">(${review.rating}/5)</span>
                                        </div>
                                        <div class="text-sm text-gray-600 mb-2">
                                            <span class="font-semibold">${typeIcon} Đối tượng:</span> 
                                            ${review.vehicle_name ? review.vehicle_name + ' (' + review.license_plate + ')' : (review.station_name || 'Hệ thống')}
                                        </div>
                ...
                .catch(error => {
                    ...
                    Swal.fire({ icon: 'error', title: 'Lỗi hệ thống', text: 'Không thể tải dữ liệu đánh giá' });
                });
        }
        ...
        function replyReview(id) {
            Swal.fire({
                ...
                html: `
                    ...
                    <textarea id="swal-input-reply" class="w-full border rounded-lg p-3 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all" rows="4" placeholder="Nhập nội dung phản hồi..."></textarea>
                `,
                ...
                preConfirm: () => {
                    const reply = document.getElementById('swal-input-reply').value;
                    if (!reply.trim()) {
                        Swal.showValidationMessage('⚠️ Vui lòng nhập nội dung trả lời');
                        return;
                    }
                    ...
                    return fetch('../../../php/admin/reply_review.php', {
                        method: 'POST',
                        body: formData
                    })
                    .then(response => {
                    ...
                    .catch(error => {
                        Swal.showValidationMessage(`Lỗi yêu cầu: ${error}`);
                    });
                },
                ...
                if (result.isConfirmed) {
                    if(result.value.success) {
                        ...
                        Swal.fire('Thất bại', result.value.message || 'Có lỗi xảy ra', 'error');
                    }
                }
            });
        }
        ...
        function hideReview(id) {
             Swal.fire({
                ...
                text: 'Đánh giá sẽ không hiển thị công khai.',
                icon: 'warning',
                showCancelButton: true,
        ...
        function clearReport(id) {
            Swal.fire({
                title: 'Xóa báo cáo?',
                text: 'Xác nhận đánh giá này không vi phạm.',
                icon: 'question',
                showCancelButton: true,
