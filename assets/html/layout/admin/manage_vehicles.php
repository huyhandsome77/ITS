            ...
            <main class="flex-1 overflow-auto p-4 md:p-6 lg:p-8" style="margin-top:76px">
                    ...
                    <section class="bg-white rounded-2xl shadow-sm border p-5 mb-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                            ...
                            <select name="vehicle_type" class="border rounded-lg px-4 py-2">
                                ...
                                <option value="Oto" <?= ($_GET['vehicle_type'] ?? '') == 'Oto' ? 'selected' : '' ?>>Ô tô
                                </option>
                                <option value="Xemay" <?= ($_GET['vehicle_type'] ?? '') == 'Xemay' ? 'selected' : '' ?>>
                                    Xe máy</option>
                            </select>

                            <select name="brand" class="border rounded-lg px-4 py-2">
                                <option value="">Tất cả hãng</option>
                                <option value="Toyota" <?= ($_GET['brand'] ?? '') == 'Toyota' ? 'selected' : '' ?>>
                                    Toyota</option>
                                <option value="Honda" <?= ($_GET['brand'] ?? '') == 'Honda' ? 'selected' : '' ?>>Honda
                                </option>
                                <option value="Mazda" <?= ($_GET['brand'] ?? '') == 'Mazda' ? 'selected' : '' ?>>Mazda
                                </option>
                                <option value="Ford" <?= ($_GET['brand'] ?? '') == 'Ford' ? 'selected' : '' ?>>Ford
                                </option>
                                <option value="Yamaha" <?= ($_GET['brand'] ?? '') == 'Yamaha' ? 'selected' : '' ?>>
                                    Yamaha</option>
                            </select>
                            ...
                            <select name="status" class="border rounded-lg px-4 py-2">
                                ...
                                <option value="AVAILABLE"
                                    <?= ($_GET['status'] ?? '') == 'AVAILABLE' ? 'selected' : '' ?>>Sẵn sàng</option>
                                <option value="RENTED" <?= ($_GET['status'] ?? '') == 'RENTED' ? 'selected' : '' ?>>Đang
                                    thuê</option>
                                <option value="MAINTENANCE"
                                    <?= ($_GET['status'] ?? '') == 'MAINTENANCE' ? 'selected' : '' ?>>Bảo trì</option>
                            </select>
                ...
                <section class="bg-white rounded-2xl shadow-sm border p-5">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <span class="text-gray-600">Tổng số: <strong
                                id="totalVehicles"><?= number_format($filteredTotal) ?></strong> xe</span>
                    </div>
                            ...
                            <tbody id="vehicleTableBody" class="divide-y">
                                ...
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-3 text-sm font-semibold">
                                        <?= htmlspecialchars($vehicle['vehicle_name']) ?></td>
                                    <td class="px-4 py-3 text-sm font-mono">
                                        <?= htmlspecialchars($vehicle['license_plate']) ?></td>
                                    <td class="px-4 py-3">
                                    ...
                                    <td class="px-4 py-3 text-sm">
                                        <?= htmlspecialchars($vehicle['station_name'] ?? 'Chưa có') ?></td>
                                    <td class="px-4 py-3 text-sm font-semibold">
                                        <?= number_format($vehicle['price_per_day']) ?>đ</td>
                                    <td class="px-4 py-3">
    ...
    <div id="vehicleModal" class="fixed inset-0 z-50 hidden overflow-y-auto bg-black bg-opacity-50">
        <div class="flex items-center justify-center min-h-screen px-4">
            <div class="bg-white rounded-2xl shadow-xl max-w-2xl w-full p-6">
                ...
                <form id="vehicleForm" method="POST" action="/ITS/assets/php/admin/add_vehicle.php"
                    enctype="multipart/form-data" class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium mb-1">Tên xe *</label>
                            <input type="text" name="vehicle_name" required class="w-full border rounded-lg px-4 py-2"
                        ...
                        <div>
                            ...
                            <select name="station_id" required class="w-full border rounded-lg px-4 py-2">
                                ...
                                <?php foreach ($stations as $st): ?>
                                <option value="<?= $st['station_id'] ?>"><?= htmlspecialchars($st['station_name']) ?>
                                </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div>
                            ...
                            <input type="text" name="license_plate" required
                                class="w-full border rounded-lg px-4 py-2 uppercase" placeholder="51G-12345">
                        </div>
                        <div>
                        ...
                        <div class="col-span-2">
                            ...
                            <textarea name="description" rows="2" class="w-full border rounded-lg px-4 py-2"
                                placeholder="Thông tin thêm về xe..."></textarea>
                        </div>
                    </div>
    ...
    <div id="editVehicleModal" class="fixed inset-0 z-50 hidden overflow-y-auto bg-black bg-opacity-50">
        <div class="flex items-center justify-center min-h-screen px-4">
            <div class="bg-white rounded-2xl shadow-xl max-w-2xl w-full p-6">
                ...
                <form id="editVehicleForm" method="POST" action="/ITS/assets/php/admin/update_vehicle.php"
                    enctype="multipart/form-data" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="vehicle_name" id="edit_vehicle_name" required
                                class="w-full border rounded-lg px-4 py-2" placeholder="Toyota Camry 2024">
                        </div>
                        <div>
                            <label class="block text-sm font-medium mb-1">Trạm *</label>
                            <select name="station_id" id="edit_station_id" required
                        ...
                        <div>
                            ...
                            <input type="text" name="brand" id="edit_brand" required
                                class="w-full border rounded-lg px-4 py-2" placeholder="Toyota, Honda...">
                        </div>
                        <div>
                            <label class="block text-sm font-medium mb-1">Model</label>
                            <input type="text" name="model" id="edit_model" class="w-full border rounded-lg px-4 py-2"
                                placeholder="Camry">
                        </div>
                        <div>
                            <label class="block text-sm font-medium mb-1">Năm sản xuất</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium mb-1">Số chỗ ngồi</label>
                            <input type="number" name="seats" id="edit_seats" min="1" max="50"
                                class="w-full border rounded-lg px-4 py-2" placeholder="5">
                        </div>
                        <div>
                            <label class="block text-sm font-medium mb-1">Giá thuê/ngày (VNĐ) *</label>
                            <input type="number" name="price_per_day" id="edit_price_per_day" required min="0"
                                class="w-full border rounded-lg px-4 py-2" placeholder="500000">
                        </div>
                        <div>
                            <label class="block text-sm font-medium mb-1">Giá thuê/giờ (VNĐ)</label>
                            <input type="number" name="price_per_hour" id="edit_price_per_hour" min="0"
                                class="w-full border rounded-lg px-4 py-2" placeholder="Tự động = giá ngày/8">
                        </div>
                        <div>
                            <label class="block text-sm font-medium mb-1">Trạng thái *</label>
