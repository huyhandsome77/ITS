                    ...
                    <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl shadow-lg p-6 text-white">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-blue-100 text-xs mt-1"><?= number_format($activeStations) ?> trạm hoạt động</p>
                            </div>
                    ...
                    <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-2xl shadow-lg p-6 text-white">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-purple-100 text-xs mt-1"><?= number_format($availableVehicles) ?> xe sẵn sàng</p>
                            </div>
                            <div class="bg-white bg-opacity-20 p-3 rounded-lg">
                                <svg class="w-8 h-8" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-2xl shadow-lg p-6 text-white">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-green-100 text-xs mt-1">+<?= number_format($newUsersThisMonth) ?> tháng này</p>
                            </div>
                            <div class="bg-white bg-opacity-20 p-3 rounded-lg">
                                <svg class="w-8 h-8" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <div class="bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-2xl shadow-lg p-6 text-white">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-yellow-100 text-xs mt-1"><?= $revenueGrowth >= 0 ? '+' : '' ?><?= number_format($revenueGrowth, 1) ?>% so với tháng trước</p>
                            </div>
                            <div class="bg-white bg-opacity-20 p-3 rounded-lg">
                                <svg class="w-8 h-8" fill="currentColor" viewBox="0 0 20 20">
                            ...
                            <?php else: ?>
                                ...
                                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                    <div class="flex items-center space-x-3">
                                        <div class="w-10 h-10 rounded-full <?= strpos($statusInfo['class'], 'blue') !== false ? 'bg-blue-100' : (strpos($statusInfo['class'], 'green') !== false ? 'bg-green-100' : 'bg-yellow-100') ?> flex items-center justify-center">
                                            <svg class="w-5 h-5 <?= strpos($statusInfo['class'], 'blue') !== false ? 'text-blue-600' : (strpos($statusInfo['class'], 'green') !== false ? 'text-green-600' : 'text-yellow-600') ?>" fill="currentColor" viewBox="0 0 20 20">
                                                <path d="M9 2a1 1 0 000 2h2a1 1 0 100-2H9z"/>
                                                <path fill-rule="evenodd" d="M4 5a2 2 0 012-2 3 3 0 003 3h2a3 3 0 003-3 2 2 0 012 2v11a2 2 0 01-2 2H6a2 2 0 01-2-2V5zm3 4a1 1 0 000 2h.01a1 1 0 100-2H7zm3 0a1 1 0 000 2h3a1 1 0 100-2h-3zm-3 4a1 1 0 100 2h.01a1 1 0 100-2H7zm3 0a1 1 0 100 2h3a1 1 0 100-2h-3z" clip-rule="evenodd"/>
                                            </svg>
                                        </div>
                                        <div>
                                            <div class="font-semibold text-sm">#DH<?= str_pad($order['order_id'], 3, '0', STR_PAD_LEFT) ?> - <?= htmlspecialchars($order['customer_name']) ?></div>
                                            <div class="text-xs text-gray-500"><?= htmlspecialchars($order['brand'] . ' ' . $order['model']) ?> • <?= $order['rental_days'] ?> ngày</div>
                                        </div>
                                    </div>
                    ...
                    <section class="bg-white rounded-2xl shadow-sm border p-5">
                        ...
                        <div class="space-y-3">
                            ...
                            <?php else: ?>
                                ...
                                ?>
                                <div class="flex items-center justify-between p-3 bg-gradient-to-r <?= $colors['bg'] ?> rounded-lg border-l-4 <?= $colors['border'] ?>">
                                    <div class="flex items-center space-x-3">
                                        <div class="w-8 h-8 rounded-full <?= $colors['badge'] ?> text-white flex items-center justify-center font-bold"><?= $rank ?></div>
                                        <div>
                                            <div class="font-semibold text-sm"><?= htmlspecialchars($station['station_name']) ?></div>
                                            <div class="text-xs text-gray-600"><?= number_format($station['total_orders']) ?> đơn • <?= number_format($station['total_revenue'] / 1000000, 0) ?>M doanh thu</div>
                                        </div>
                                    </div>
                                </div>
                                <?php 
                ...
                <section class="bg-white rounded-2xl shadow-sm border p-5">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex items-start space-x-3 p-3 bg-red-50 border border-red-200 rounded-lg">
                            ...
                            <div class="flex-1">
                                <div class="font-semibold text-sm text-red-900"><?= number_format($maintenanceVehicles) ?> xe đang trong tình trạng bảo trì</div>
                        ...
                        <div class="flex items-start space-x-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                            ...
                            <div class="flex-1">
                                <div class="font-semibold text-sm text-yellow-900"><?= number_format($pendingPayments) ?> giao dịch chờ xác nhận</div>
                        ...
                        <div class="flex items-start space-x-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                            ...
                            <div class="flex-1">
                                <div class="font-semibold text-sm text-blue-900"><?= number_format($pendingReviews) ?> đánh giá đang chờ duyệt</div>
